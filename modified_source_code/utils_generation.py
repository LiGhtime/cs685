...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # customized constraint logits processor
        if (
            generation_config.asin_dict is not None
        ):
            # flag for customized constraint logits processor
            logger.warning("Using customized constraint logits processor")
            processors.append(
                ConstraintLogitsProcessor(
                    prompt_length_to_skip=input_ids_seq_length, eos_token_id=generation_config.eos_token_id, 
                    bos_token_id=generation_config.bos_token_id,
                    asin_dict=generation_config.asin_dict,
    ...
    def _beam_sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # # if logits_processor is our customized ConstraintLogitsProcessor(LogitsProcessor):
            # if isinstance(logits_processor, ConstraintLogitsProcessor):
            #     # call constraint logits processor
            #     next_token_scores = logits_processor(input_ids, next_token_scores)
            # else:
            next_token_scores_processed = logits_processor(input_ids, next_token_scores) # TODO: maybe need to add trie here
