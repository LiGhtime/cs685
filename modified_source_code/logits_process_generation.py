...
class ConstraintLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] enforcing a constraint on the output tokens. At each step, the constraint is evaluated and truncate the 
    vocabulary to only keep tokens that satisfy the constraint. The constraint is a trie that contains all the possible tokens 
    in a tree structure.
    ...
    Args:
        ...
        eos_token_id (`Union[int, List[int]]`):
            The id of the *end-of-sequence* token. Optionally, use a list to set multiple *end-of-sequence* tokens.
        bos_token_id (`Union[int, List[int]]`):
            The id of the *beginning-of-sequence* token. Optionally, use a list to set multiple *beginning-of-sequence* tokens.
        asin_dict (`Dict[str, List[str]]`):
            A dictionary that contains the ASINs as keys and the movie titles (at [0]) and descriptions (at [1]) as values.
        tokenizer (`Callable`):
            The tokenizer function that tokenizes the movie titles.
    """
    
    def __init__(self, prompt_length_to_skip: int, eos_token_id: Union[int, List[int]], bos_token_id: Union[int, List[int]], asin_dict: Dict[str, List[str]], tokenizer: Callable):
        for arg_name, arg_value in [
            ("prompt_length_to_skip", prompt_length_to_skip),
        ]:
            if not isinstance(arg_value, int) or arg_value < 0:
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # The trie is constructed from movie titles, which are encoded into tokens. 
        # The root of the trie is the start_of_sentence token. The children of the root, 
        # or the first level of the trie, consist of possible tokens that can follow the start_of_sentence token in our 
        # movie title corpus. The function traverses the trie and if the current token is not in the trie, it sets the 
        # score to negative infinity. This is done to ensure that only valid sequences of tokens (i.e., those that form 
        # valid movie titles) have non-negative scores at each generation step.
        
        # input_ids is the current sequence of tokens generated by the model
        # we can use it to traverse the trie and find the children of the current token
        # search for the new_token_input_ids in the trie, to get the children of the current token
        current_node_list = []
        for i in range(num_beams):
        ...
        # for token in vocab_tensor:            
        #     # check if the current token is in the children of the current node
        #     if token.item() not in current_node.children:
        #         not_in_children_token_mask = torch.isin(vocab_tensor, token)
        #         scores_processed = torch.where(not_in_children_token_mask, -math.inf, scores_processed)
        for i, current_node in enumerate(current_node_list):
            for token in current_node.children:
                ...
                # print("scores dim: ", len(scores.size()))
                # print("len of the first dim of scores: ", num_beams)
                if num_beams == 1:
                    # if the token is in the children of the current node, set the score to the original score
                    token = torch.tensor(token, device=scores.device)     
                    original_scores = scores[:, token] 
                    # print("original_scores: ", original_scores)
                    # print("original_scores size: ", original_scores.size())                  
                    ...
                    # print("beam_score size: ", beam_score.size())
                    original_scores = scores[i][token].unsqueeze(0)
                    # print("original_scores: ", original_scores)  
                    # print("original_scores size: ", original_scores.size())                 
                    in_children_token_mask = torch.isin(vocab_tensor, token)
                    # print("in_children_token_mask: ", in_children_token_mask)
                    scores_processed[i] = torch.where(in_children_token_mask, original_scores, scores_processed[i])

        return scores_processed.to(torch.float16) # type is hard coded for now
