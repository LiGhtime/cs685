    ...
    def _CausalLM_fast_forward(
        ...
        *args, **kwargs,
        ...
        if bsz == 1 and q_len == 1:
            ...
            # customized lm_head dtype
            # take average of hidden_states over the second dim (q_len)
            averaged_hidden_states = hidden_states.mean(dim = 1)
            # get rid of the second dim
            averaged_hidden_states = averaged_hidden_states.unsqueeze(1)
            logits = self.lm_head(averaged_hidden_states.to(lm_head.multihead_attn.out_proj.weight.dtype))
        pass
